...
// const teamNames = 'C:\Users\mcwho\OneDrive\Documents\IS Winter 2023\IS 413\Mission12_2\frontend\public\CollegeBasketballTeams.json';

// fetch(teamNames)
//   .then(response => response.json())
//   .then(data => {
//     const myJsonData = data;
...
function Description() {
  return (
    <p>These are all the teams that play college basketball</p>
  );
}
class Team extends React.Component<{ name: string; school: string; city: string; state: string;  }> {
  system = { counter: 1 }
  render() {
    const { counter } = this.system;
    const singleTeam = this.props;
    return (
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        <div style={{flex: "0 0 5%"}}>{counter}</div>
        <div style={{ flex: 1 }}>{singleTeam.name}</div>
        <div style={{ flex: 1 }}>{singleTeam.school}</div>
        <div style={{ flex: 1 }}>{singleTeam.city}</div>
        <div style={{ flex: 1 }}>{singleTeam.state}</div>
      </div>
    );
...
function TeamList() {
  return (
    <div>
      {teamNames.map(singleTeam => <Team {...singleTeam}/>)}
    </div>
  );
}
function App() {
